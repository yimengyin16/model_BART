...
- Add employer contribution as a percentage of projected budget to the result tables
- Additional policy options after our discussion on Oct 6

## Key results {.tabset} 

**CAUTION**: 

For all policies below involving COLA suspension/reduction until full funding, we assume that the CalPERS actuary would reduce the assumed COLA rate immediately after the reform to reflect the fact that COLA will stay low for an extended period of time. As a result, the actuarial liability and ADC, and therefore employer contribution, would decrease immediately after the reform. 

A potential alternative actuarial treatment of COLA suspension/reduction by the actuary would be to keep the original COLA assumption (2% for BART) and let the COLA suspension/reduction create actuarial gains (actual COLA is lower than assumed COLA) in subsequent years. With this treatment, immediately after the reform the plan would see little decrease in actuarial liability and required contribution due to the COLA suspension/reduction, but the liability and required contributions would decrease over time as the actuarial gains build up.

Thus, the numbers presented in the table below show the best short-term fiscal outcome for BART because we assume employer contributions can be reduced immediately. Under the alternative possible assumption, fiscal benefit for BART would be far smaller in the short run and larger in the long run. Over the very long, both actuarial approaches would have the same impact.

We will investigate which actuarial treatment of COLA suspension/reduction is more consistent with what plan actuaries would actually do in practice.
...
There are two BART plans, one for miscellaneous employees and one for safety (police) members. The miscellaneous plan is far larger than the safety plan. Both are participating plans of CalPERS.   
...
return_label <- switch (as.character(sim_select),
  ...
  "-2" = "Asset shock: 3% in 2019, 11% in 2020, negative 24% in 2021, then 3 years of 12% (2022-2024), then 7% annually"
)
...
**Miscellaneous members**:
 ...
 2. Misc mid-career active:    age 45 (Classic, half of career is affected)
...
**Safety members**:
 ...
 2. Safety mid-career active:    age 40 (Classic, half of career is affected)
...
tbl_ben_age80_misc <- 
  ...
  locations = cells_column_labels()
   ...
   locations = cells_title(groups = "title")
    ...
    subtitle = html("Each cell shows benefit adjusted for inflation (in 2019 dollars, assuming 2.5% inflation rate);<br>Assuming 7% return every year")
  ) 
...
file.copy(paste0(here::here(),"/analysis/Results_BART(4).nb.html"), paste0(here::here(), "/docs/Results_BART(4).nb.html"), overwrite = TRUE)
