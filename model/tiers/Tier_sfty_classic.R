...
#' What this file does
#'   - produce the following model inputs for tier "sfty_classic"
#'     - decrement table
#'     - salary scale
...
#'  
#'  - Benefit rules
#'      - Use benefit rules based on Misc classic members with 3%@50 rules.
#'        (The 3%@50 rule is applied to all classic members who joined before 1/15/2011, 
...
#  
#  - Based on standard industrial disability retirement benefit
#  
#  - Eligibility: no requirement
#
#  - Simplification: 
#      - assumming all are industrial disability retirement
...
# 90% male and 10% female for all calculations for sfty members. 
...
EEC_rate <- 0.09  # TODO:check/estimate EEC rate for classic and PEPRA members
...
grp_include <- grp_include[str_detect(grp_include , "sfty_classic")]

# weight for each group
wgts <- tibble(grp = grp_include, wgt = 0)

wgts[wgts$grp == "sfty_classic","wgt"] <-  1
...
## Use total disability rates (ind + non-ind)
## and assume all are industrial disability
...
grp_include <- df_qxt.refund_sfty_imputed$grp %>% unique
grp_include <- grp_include[str_detect(grp_include, "sfty")]

# weight for each group
wgts <- tibble(grp = grp_include, wgt = 0)

wgts[wgts$grp == "sfty",  "wgt"] <-  1
...
# groups included
grp_include <- df_qxt.vest_sfty_imputed$grp %>% unique
grp_include <- grp_include[str_detect(grp_include, "sfty")]
...
df_qxt_tier <- 
  ...
  relocate(grp, yos, qxt)


## Pre-retirement mortality
# For BART safty plan, use total pre-retirement mortality (ind + non-ind)

df_qxm.pre_tier <-  
  df_qxm.pre_imputed %>% 
  mutate(qxm.pre.nonocc = share_female * qxm.pre.nonocc_female + share_male * qxm.pre.nonocc_male,
         qxm.pre.occ    = share_female * qxm.pre.occ_female    + share_male * qxm.pre.occ_male,
...
#  - assume all of disability retirement is job-related for BART safety
...
df_qxm.post_tier <-  
  ...
  mutate(qxm.post         = share_female * qxm.post_female         + share_male * qxm.post_male,
         ...
         qxmd.post.occ    = share_female * qxmd.post.occ_female    + share_male * qxmd.post.occ_male,
         qxmd.post        = 0 * qxmd.post.nonocc + 1 * qxmd.post.occ,
         grp = tier_name
  ) %>% 
  select(grp, age, 
         qxm.post, qxmd.post
...
df_qxm.post_proj_tier <-  
  ...
  mutate(qxm.post_proj         = share_female * qxm.post_female_proj         + share_male * qxm.post_male_proj,
         ...
         qxmd.post.occ_proj    = share_female * qxmd.post.occ_female_proj    + share_male * qxmd.post.occ_male_proj,
         qxmd.post_proj        = 0 * qxmd.post.nonocc_proj + 1 * qxmd.post.occ_proj,
         grp = tier_name
  ) %>% 
  select(grp, age, 
...
decrements_tier <- expand.grid(age = range_age, 
  ...
  left_join(df_qxt_tier,            by = c("grp", "yos")) %>%   # termination with vested benefit
...
decrements_tier  %<>% 
  ...
  mutate(
    ...
    elig_servRet_full = ifelse( (age >= 50 & yos >= 5), 1, 0) %>% cumsum,
...
# BART safety: Check total benefit againt the AV value 
...
# model/target:  11904589 / 11904589 = 1
...
# model/target: 6753677/v6753677 = 1%
