# This script calculates individual liabilities and normal costs
# BART 2019
...
liab_active %<>%   
	mutate(
		...
		# 	grp %in% c("misc_classic") & age %in% 50:55 ~ 1 - (55 - age) * 0.09,
		# 	grp %in% c("misc_classic") & age %in% 56:63 ~ 1 + (age - 55) * 0.03125,
		# 	grp %in% c("misc_classic") & age > 63 ~ 1.25,
		# 	
...
# BART
  ...
  # - Use benefit factor (benReduction) for age_vben (age at which the member is assumed to start receiving benefits)
...
liab_active %<>% 
  mutate(gx.defrRet = yos >= v.year,   # actives become vested after reaching v.yos years of yos
  			 ...
  			 Bx.defrRet = ifelse(ea < age_vben, 
  			                     #gx.defrRet * Bx * gx.servRet.laca,   
  			 										 gx.defrRet * Bx * benReduction[age == age_vben],   
...
liab_active %<>% 
  mutate( gx.disbRet  = case_when(
                          grp %in% c("misc_classic", "misc_pepra") ~ as.numeric(yos >= 5),
                          grp %in% c("sfty_classic", "sfty_pepra") ~ 1,
                          TRUE ~ 0
                          ),
          ...
          # yos_proj_disbRet = ifelse(age <= 60, 60 - ea, yos), 
  				yos_proj_disbRet = case_when(
  				  yos < 10 | yos > 18 ~ yos,
  				  TRUE ~ 60-ea  # years the member would have worked until age 60
  				), 
  				
  			
  				Bx.disbRet  = case_when(
  				    # PERF A non-industrial disability benefit policy
  				    grp %in% c("misc_classic", "misc_pepra") ~ gx.disbRet * pmin(1/3 * fas, 0.018 * yos_proj_disbRet * fas, na.rm = TRUE),
  				    
  				    # PERF A industrial disability benefit policy
  				    grp %in% c("sfty_classic", "sfty_pepra") ~ gx.disbRet * 0.5 * fas,
  				    TRUE ~ 0
  				  ),
...
liab_active %<>%
	mutate( gx.death  = 1,
					...
					Bx.death = fct_deathBen * sx * gx.death,   # pmax(0, gx.death * sx * 3 - 50000),
